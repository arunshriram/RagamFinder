...
def convertMIDIToNote(noteNumber):
    return pitchDictionary[noteNumber%12]
...
while True:
    ...
    if(note not in pitch_frequencies):
        pitch_frequencies[note] = 1
    else:
        pitch_frequencies[note] += 1
...
print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
print()
print("Total number of pitches found: %d" % len(pitches))
print()
print("Here are the pitches and their respective frequencies:")
print(sorted(pitch_frequencies.items(), reverse=True, key = lambda kv: (kv[1], kv[0])))
#print(pitches)
